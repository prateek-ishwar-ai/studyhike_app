...
export function AdminHeader({ adminName }: AdminHeaderProps) {
  ...
  return (
    <header className="bg-[#111420] border-b border-gray-800 h-16 flex items-center justify-between px-6">
      <div>
        <h1 className="text-xl font-semibold text-gray-100">Admin Dashboard</h1>
      </div>
          ...
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="relative h-8 w-8 rounded-full hover:bg-[#252A3D]">
              <Avatar className="h-8 w-8">
                <AvatarFallback className="bg-yellow-400 text-[#0C0E19]">{adminName.charAt(0)}</AvatarFallback>
              </Avatar>
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent className="w-56 bg-[#1A1D2D] border border-gray-700 text-gray-200" align="end" forceMount>
            <div className="flex items-center justify-start gap-2 p-2">
              <div className="flex flex-col space-y-1 leading-none">
                <p className="font-medium text-gray-200">{adminName}</p>
                <p className="w-[200px] truncate text-sm text-gray-400">admin@studyhike.com</p>
              </div>
            </div>
